    ...
    public SpringObjenesis(InstantiatorStrategy strategy) {
        this.strategy = strategy != null ? strategy :
            new InstantiatorStrategy() {
                private final org.objenesis.strategy.InstantiatorStrategy strategy = new org.objenesis.strategy.StdInstantiatorStrategy();

                @Override
                public <T> ObjectInstantiator<T> newInstantiatorOf(final Class<T> aClass) {
                    return new ObjectInstantiator<T>() {
                        private final org.objenesis.instantiator.ObjectInstantiator<T> instantiator = strategy.newInstantiatorOf(aClass);
                        @Override
